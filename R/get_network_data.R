...
#' format, with each row a state dyad combination. The parameter \code{merge}
...
get_network_data <- function(category = NULL, merge_data = NULL){
  ...
  if(!is.null(category) & !all(category %in% c("Distance Travel Migration", "Economic", "Political", "Policy", "Demographic"))) {
    stop('Category must be one of "Distance Travel Migration", "Economic", "Political", "Policy", "Demographic"')
  }
